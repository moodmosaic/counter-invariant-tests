...
import { Cl, cvToJSON, uintCV } from "@stacks/transactions";
...
const sutContracts: string[] = [
  "ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.counter_flow_test",
];

it("run invariant testing", () => {
  const availableFunctions = getContractFunctions(simnet, sutContracts);
    ...
    return fc.assert(
      fc.property(argsArb, (args) => {
        ...
        allInvariants.forEach((invariant) => {
          ...
          expect(testInvariant).toBeOk(Cl.bool(true));
        });
      }),
